...
3. [기본 사용법](#기본-사용법)
4. [🎯 수동 조정 기능](#-수동-조정-기능)
5. [파일 형식 이해하기](#파일-형식-이해하기)
6. [❓ 자주 묻는 질문](#-자주-묻는-질문)
7. [🔧 문제 해결 가이드](#-문제-해결-가이드)
8. [부록: 개발자를 위한 정보](#부록-개발자를-위한-정보)
...
**macOS 사용자**
- `학급편성.app` 더블클릭
- 보안 경고 나오면: 시스템 설정 → 개인정보 보호 → "확인 없이 열기"
...
│  반 목록     │    학생 목록        │  통계 정보   │
├──────────────┼─────────────────────┼──────────────┤
│ 6-1반 (20명) │ 🟡 김철수 (남)      │ 총 학생: 20  │
│ 6-2반 (21명) │    분반: 이민준(3반)│ 유효: 22.0   │
│ 6-3반 (23명) │                     │ 남: 8명      │
│ 6-4반 (22명) │ 이영희 (여)         │ 여: 12명     │
│ 6-5반 (22명) │                     │ 특수반: 1명  │
│ 6-6반 (22명) │ 🔴 박민수 (남)      │ 난이도: 5.0  │
│ 6-7반 (22명) │    특수반           │              │
│              │                     │              │
│              │ 🔵 최지우 (여)      │ [범례]       │
│              │    합반: 박서준     │ 🔴 특수반    │
...
**1단계: 반 선택**
- 왼쪽 패널에서 학생이 현재 속한 반 클릭

**2단계: 학생 선택**
- 가운데 패널에서 이동할 학생 클릭
- (선택) 검색창에 이름 입력하여 빠르게 찾기

**3단계: 이동 버튼 클릭**
- **"→ 다른 반으로 이동"** 버튼 클릭

**4단계: 대상 반 선택**
- 시스템이 자동으로 **이동 가능한 반만** 보여줍니다
- 원하는 반 선택

**5단계: 경고 확인 (있는 경우)**
- 합반 그룹 학생을 이동하면 경고 메시지가 나타남
- 그룹 구성원 확인 후 계속 진행 여부 결정
...
시스템이 자동으로 다음 사항을 검사하여 **이동 가능한 반만** 표시합니다:

✅ **분반 규칙 검사**: 분반해야 하는 학생이 있는 반은 제외
✅ **동명이인 검사**: 같은 이름 학생이 있는 반은 제외
✅ **합반 규칙 검사**: 합반 그룹 멤버가 없는 반은 경고 표시

### ⚠️ 중요한 주의사항

1. **규칙은 자동으로 지켜집니다**
   - 분반/합반 규칙을 어기는 이동은 **차단**됩니다
   - 규칙을 위반하는 반은 선택지에 나타나지 않습니다

2. **합반 그룹 분리 경고**
   - 합반 그룹 학생을 이동하면 경고가 나타납니다
   - 그룹 전체를 이동하려면 각 학생을 **하나씩** 이동시켜야 합니다

3. **저장 필수**
   - 조정 후 반드시 **"📥 Export to Excel"** 클릭하여 저장하세요
   - 저장하지 않으면 **조정 내용이 사라집니다**

4. **통계는 자동 계산됨**
   - 학생 수, 성비, 난이도 등은 자동으로 업데이트됩니다
   - 오른쪽 통계를 보면서 균형을 맞추세요
