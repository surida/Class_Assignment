...
4. [기본 사용법](#기본-사용법)
5. [📋 전체 보기 기능 (Overview)](#-전체-보기-기능-overview)
6. [🎯 수동 조정 기능](#-수동-조정-기능)
7. [파일 형식 이해하기](#파일-형식-이해하기)
8. [❓ 자주 묻는 질문](#-자주-묻는-질문)
9. [🔧 문제 해결 가이드](#-문제-해결-가이드)
10. [부록: 개발자를 위한 정보](#부록-개발자를-위한-정보)
...
### 4. 수동 배정 및 조정 (New)
* **드래그 앤 드롭 지원**:
    * **같은 반 내부 이동**: 목록에서 학생을 드래그하여 원하는 순서로 변경 가능합니다. (번호 자동 갱신)
    * **반 간 이동**: 학생을 다른 반 목록의 특정 위치로 드래그하여 정확한 순서에 삽입할 수 있습니다.
* **순서 유지**:
    * 수동으로 조정한 순서는 **저장 시 그대로 엑셀 파일에 반영**됩니다.
    * 제출용 시트(5-n)도 수동 조정된 번호와 동일하게 생성됩니다.
    * *초기 배정 시에만 이름순 정렬되며, 이후에는 사용자 지정 순서가 최우선됩니다.*

## 5. 엑셀 출력
* **메인 배정 결과**: 각 반별 탭에 배정된 학생 명단 (수동 조정 순서 반영)
* **제출용 시트 (5-n)**: 5학년 반 기준으로 6학년 배정 결과를 정리한 시트 (번호 일치)
* **요약/규칙 시트**: 배정 통계 및 적용된 규칙 확인 가능
...
### 방법 2.1: 실행 스크립트 사용 (권장) ⭐⭐
가상환경(`venv`) 설정 실수 등으로 실행이 안 될 경우, 동봉된 실행 스크립트를 사용하면 시스템 Python을 사용하여 안정적으로 실행할 수 있습니다.
...
모든 학급의 학생 배치를 **한눈에 확인**하고 **빠르게 조정**할 수 있는 칸반(Kanban) 스타일 화면입니다.

### 언제 사용하나요?

- ✅ **전체 현황 파악**: 7개 반 전체 학생 배치를 한 화면에서 확인하고 싶을 때
- ✅ **빠른 조정**: 특정 학생을 다른 반으로 빠르게 이동하고 싶을 때
- ✅ **반 간 비교**: 두 반을 선택하여 상세 비교 및 수동 조정하고 싶을 때
...
│(22명) │ │(22명) │ │(23명) │ │(20명) │ │(20명) │ │(23명) │ │(22명) │
├───────┤ ├───────┤ ├───────┤ ├───────┤ ├───────┤ ├───────┤ ├───────┤
│🟣김민준│ │⚪이서연│ │🟡박지호│ │⚪최수아│ │🔵정도윤│ │⚪한서윤│ │⚪윤시우│
│⚪이하은│ │🟡강민서│ │⚪오예준│ │⚪장하린│ │🔵서예은│ │🟣문지안│ │⚪임채원│
│  ...  │ │  ...  │ │  ...  │ │  ...  │ │  ...  │ │  ...  │ │  ...  │
└───────┘ └───────┘ └───────┘ └───────┘ └───────┘ └───────┘ └───────┘
...
**특징**:
- **칸반 레이아웃**: 각 반이 세로 컬럼으로 표시
- **컬럼 헤더**: 반 번호, 총 인원, 남녀 비율 표시
- **색상 표시**: 학생 상태(특수/분반/합반/전출)를 색상으로 구분
- **반응형 스크롤**: 학생이 많아도 각 컬럼 내에서 스크롤 가능

### 학생 이동 방법 (드래그 앤 드롭)

**Overview에서 직접 이동**:
1. 이동할 학생 카드를 **마우스로 클릭**하고 드래그 시작
2. 목표 반 컬럼으로 **드래그하여 이동**
3. 삽입할 위치에서 **파란 하이라이트** 표시 확인
4. 원하는 위치에서 **마우스 버튼 놓기** → 이동 완료

**위치 지정 삽입**:
- 드래그 중 학생 카드 **위쪽**에 마우스를 놓으면 그 학생 **앞에** 삽입
- 드래그 중 학생 카드 **아래쪽**에 마우스를 놓으면 그 학생 **뒤에** 삽입
- 빈 공간에 놓으면 **맨 뒤**에 추가

### 수동 조정 화면으로 이동

더 상세한 조정이 필요하면 **두 반을 선택**하여 수동 조정 화면으로 이동할 수 있습니다.

1. **첫 번째 반 클릭** → 선택됨 (테두리 강조)
2. **두 번째 반 클릭** → 두 반 선택 완료
3. **"🔧 수동 조정 화면으로"** 버튼 클릭
4. 선택한 두 반이 양쪽에 표시된 수동 조정 화면 열림

**팁**:
- 반을 다시 클릭하면 선택 해제됩니다
- 이미 2개 선택된 상태에서 새 반을 클릭하면 첫 번째 선택이 해제됩니다

### 저장하기

조정 후 반드시 **"💾 저장"** 버튼을 클릭하여 Excel 파일로 저장하세요.
...
- 🟣 **보라 (Purple)**: 특수반 학생 (Special Needs)
- 🟡 **노랑 (Gold)**: 분반 규칙 학생 (특정 학생과 **다른 반**이어야 함)
- 🔵 **파랑 (Blue)**: 합반 규칙 학생 (특정 학생과 **같은 반**이어야 함)
- ⚫ **회색 (Gray)**: 전출 예정 학생 (Transferred)
- ⚪ **흰색 (White)**: 일반 학생 (Normal)

**복합 상태 (Mixed Status)**:
- 두 가지 이상의 상태를 가진 학생은 **아이콘이 나란히** 표시됩니다.
- 예: 전출 + 분반 → ⚫🟡 (회색 + 노랑 아이콘)
...
| 학년 | 반 | 번호 | 이름 | 성별 | 점수 | 특수반 | 전출 | 난이도 | 정보 | 비고 | 원학년 | 원반 | 원번호 |
|-----|---|-----|-----|-----|-----|-------|-----|-------|-----|-----|-------|-----|-------|
| 6 | 1 | 1 | 강민준 | 남 | 92 | | | | | | 5 | 3 | 12 |
| 6 | 1 | 2 | 김서연 | 여 | 90 | 1 | | 2.0 | ●(보라) | 느티모반 | 5 | 1 | 8 |

**특징**:
- 학생은 **이름 가나다순**으로 정렬
- **정보(Info) 컬럼** ⭐: 학생의 상태(특수, 전출, 분반, 합반)를 **색깔 원(●)** 아이콘으로 표시
    - 엑셀에서도 수동 조정 화면과 동일한 시각적 정보를 확인할 수 있습니다.
- **원학년, 원반, 원번호**: 이전 학년 정보 (학생 추적용)
- **정보(Info) 컬럼** ⭐: 학생의 상태(특수, 전출, 분반, 합반)를 **색깔 원(●)** 아이콘으로 표시
    - 엑셀에서도 수동 조정 화면과 동일한 시각적 정보를 확인할 수 있습니다.
- **원학년, 원반, 원번호**: 이전 학년 정보 (학생 추적용)
- 분반 규칙 학생 셀에 **메모** 자동 추가

#### 이전 학년 제출용 시트 (NEW)
**5-1, 5-2, ...** 시트가 추가로 생성됩니다.
- 학교 제출용 출석부 양식에 맞게 **이전 학년 반/번호** 순으로 정렬되어 있습니다.
- **배정반, 배정번호** 컬럼을 통해 학생들이 어떤 반으로 배정되었는지 한눈에 확인할 수 있습니다.
...
### 문제 3: 프로그램이 갑자기 종료됩니다 (크래시)

**증상**:
- "파일을 읽는 중입니다..." 후에 갑자기 창이 닫힘
- 오류 메시지 없이 종료됨

**원인 및 해결**:
1. **로그 파일 확인**:
   - 프로그램이 실행된 폴더에 `class_assigner_debug.log` 파일이 생성됩니다.
   - 이 파일을 열어서 `ERROR` 라고 표시된 부분을 확인하세요.

2. **일반적인 원인**:
   - **Excel 파일 손상**: 파일이 열려있는 상태에서 실행했거나, 파일이 깨진 경우
   - **필수 컬럼 누락**: 학생 명단에 '이름', '성별' 등 필수 열이 없는 경우
   - **규칙 파일 형식 오류**: 규칙 파일의 시트 이름이 다르거나 열이 부족한 경우 (최신 버전에서 해결됨)

3. **해결 방법**:
   - 실행 중인 모든 Excel 파일을 닫고 다시 실행하세요.
   - 최신 버전의 프로그램을 사용하세요 (v2.7 이상 권장).
   - 문제가 지속되면 `class_assigner_debug.log` 파일을 개발자에게 보내주세요.

---

### 문제 4: 결과가 이상합니다 (한 반에 학생이 너무 적거나 많음)
...
**v2.8 (2025-12-31)**
- ✨ **전체 보기(Overview) 화면 추가**: 모든 반을 칸반(Kanban) 스타일로 한눈에 확인
- ✨ **Overview 드래그 앤 드롭**: 전체 보기에서 직접 학생을 다른 반으로 이동 가능
- ✨ **위치 지정 삽입**: 드롭 시 삽입 위치를 시각적으로 표시하고 원하는 위치에 정확히 삽입
- ✨ **화면 간 데이터 동기화**: Overview와 수동 조정 화면 간 변경사항 실시간 공유
- 🎨 **시스템 테마 지원**: OS 테마(라이트/다크)를 자동으로 따르도록 개선

**v2.7 (2025-12-28)**
- ✨ **수동 배정 기능 강화**: 드래그 앤 드롭으로 정확한 위치에 학생 삽입 가능
- ✨ **순서 유지**: 수동으로 변경한 순서가 저장 및 제출용 시트에 반영되도록 개선
- 🐛 **화면 시인성 개선**: Windows에서 글자가 잘 보이지 않던 문제 해결
- 🐛 **제출용 시트 동기화**: 5-n 시트 번호 오류 수정
- 🐛 **안정성**: 실행 로직 오류 수정

**v2.6 (2025-12-21)**
- ✨ 버전 정보 표시: 모든 화면에 버전 번호 추가
- 🐛 Windows 호환성 개선: 라벨 색상 명시화
- ✅ 수동 조정 기능 테스트 추가
- 🐛 PyInstaller import 오류 수정
