...
- 기존 반(5-1~5-7) 학생들이 새 학급에 골고루 분산되도록 배치
- **기존 반 처리 순서 랜덤 생성**: 예 [2, 1, 5, 7, 4, 3, 6]
- 각 기존 반별로 남녀 교차 처리:
  1. 5-2반 남학생들(점수순) → 6-1, 6-2, 6-3, 6-4, 6-5, 6-6, 6-7 순환 배정
  2. 5-2반 여학생들(점수순) → (바로 이어서) 6-1, 6-2, ...
  3. 5-1반 남학생들 → (이어서)
  4. 5-1반 여학생들 → (이어서)
  5. ... 5-6반까지 반복
- 효과: 기존 반 균등 분산 + 성비 균형 + 성적 균형
- 분반 규칙 준수
